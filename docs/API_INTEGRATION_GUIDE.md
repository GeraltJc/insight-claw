...
历史联调记录不再写入公开文档；以当前仓库的 Actions 运行结果为准：
...
| `AIHUBMIX_KEY` | AIHubMix key 的镜像别名，便于本地运行复用 |
...
- `AIHUBMIX_KEY` 作为镜像别名保留，主要是为了本地运行时少做映射转换
...
如果在新 session 里做自动接入，建议先按“能力包”接入，不要直接把整仓库当作 Skill。
