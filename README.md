...
[功能架构](docs/FUNCTION_ARCHITECTURE.md) · [快速开始](docs/QUICKSTART_ARCHITECTURE.md)

</div>

## 1. 能力简介

`Insight Claw` 是一个面向 A 股自选股的自动化分析流水线，覆盖：
...
```mermaid
flowchart LR
    A["输入自选股<br/>CLI / GitHub Actions / 定时任务"] --> B["行情与基本面<br/>日线 / 实时 / 筹码 / iFinD 增强"]
    B --> C["搜索增强<br/>新闻 / 舆情 / 业绩 / 行业"]
    C --> D["LLM 结构化分析<br/>结论 / 风险 / 计划"]
    D --> E["生成报告<br/>单股报告 + 批次汇总"]
    E --> F["发送结果<br/>Telegram / 飞书 / 其它通道"]

    B -. "按可用性自动降级" .-> B1["Tushare / Efinance / Akshare / ..."]
    C -. "多源补充" .-> C1["Bocha / Tavily / SerpAPI"]
    D -. "主模型失败自动回退" .-> D1["LiteLLM fallback models"]
    F -. "未配置通知时" .-> F1["仅保存本地报告"]
```

一句话理解：给它一组股票代码，它会自动收集行情和外部信息，让模型生成可执行的分析结果，并保存或推送出去。
...
- Insight Claw 会优先发送飞书卡片消息，失败时自动回退为普通文本
