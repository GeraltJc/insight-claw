...
class SearchService:
    ...
    def search_comprehensive_intel(
        ...
        if is_foreign:
            search_dimensions = [
                {'name': 'latest_news', 'query': f"{stock_name} {stock_code} latest news events", 'desc': '最新消息', 'days': 7},
                {'name': 'market_analysis', 'query': f"{stock_name} analyst rating target price report", 'desc': '机构分析', 'days': 30},
            ...
            search_dimensions = [
                {'name': 'latest_news', 'query': f"{stock_name} {stock_code} 最新 新闻 重大 事件", 'desc': '最新消息', 'days': 7},
                {'name': 'market_analysis', 'query': f"{stock_name} 研报 目标价 评级 深度分析", 'desc': '机构分析', 'days': 30},
                {'name': 'risk_check', 'query': (
        ...
        for dim in search_dimensions:
            ...
            search_days = int(dim.get('days', self.news_max_age_days))
            logger.info(f"[情报搜索] {dim['desc']}: 使用 {provider.name}, 近{search_days}天")
            
            response = provider.search(dim['query'], max_results=5, days=search_days)
    ...
    @staticmethod
    def _is_low_signal_search_result(result: SearchResult) -> bool:
        """Return True for quote/discussion pages that add little news value."""
        title = (result.title or "").lower()
        snippet = (result.snippet or "").lower()
        source = (result.source or "").lower()
        url = (result.url or "").lower()
        combined = "\n".join([title, snippet, source, url])

        discussion_markers = ("股吧", "分析讨论社区")
        ...
        if any(marker in combined for marker in quote_title_markers):
            return True

        market_data_markers = ("当前:", "现手:", "换手率:", "量比:", "涨停:", "市盈ttm")
        market_data_hits = sum(1 for marker in market_data_markers if marker in combined)
        return market_data_hits >= 3

    def _filter_low_signal_results(self, results: List[SearchResult]) -> List[SearchResult]:
        filtered = [result for result in results if not self._is_low_signal_search_result(result)]
        removed_count = len(results) - len(filtered)
        if removed_count > 0:
            logger.debug("过滤低信号搜索结果 %d 条（行情页/股吧/讨论页）", removed_count)
        return filtered
    ...
    def format_intel_report(self, intel_results: Dict[str, SearchResponse], stock_name: str) -> str:
        ...
        for dim_name in display_order:
            ...
            display_results = self._filter_low_signal_results(resp.results) if resp.success else []
            if display_results:
                # 增加显示条数
                for i, r in enumerate(display_results[:5], 1):
                ...
                lines.append("  未找到有效情报（已过滤行情页/股吧/讨论页等低信号结果）")
